let
  ...
  typeCheck =
    ...
    if test x then
      ...
      builtins.abort ("NixVirt: expected " + tname + ", found " + builtins.typeOf x);

  typeConvert =
    tname: test: conv: x:
    conv (typeCheck tname test x);

  attrOrNull = a: subject: if builtins.hasAttr a subject then builtins.getAttr a subject else null;
...
rec {
  ...
  elem =
    ...
    xml.elem etype (map (a: a subject) attrs) (
  ...
  attr =
    atype: contents: subject:
    xml.attr atype (contents subject);

  subattr = atype: contents: sub atype (attr atype contents);

  subelem =
    etype: attrs: contents:
    sub etype (many (elem etype attrs contents));
  ...
  typePath = typeConvert "path or string" (
    x: builtins.isPath x || builtins.isString x
  ) builtins.toString;
}
