...
// {
  ...
  devices = base.devices // {
    disk =
      (if builtins.isNull storage_vol then [ ] else [ (basestuff.mkstorage virtio_drive storage_vol) ])
      ++ [
        {
          type = "file";
          device = "cdrom";
          ...
          source =
            if builtins.isNull install_vol then
              null
            else
              {
                file = install_vol;
          ...
          [
            {
              ...
              source = {
                file = "${guest-install.virtio-win.iso}";
              };
              target = {
                bus = "sata";
                dev = "hdd";
